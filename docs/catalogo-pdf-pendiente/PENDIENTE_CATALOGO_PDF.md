...
- La funcionalidad existente vive en `src/lib/lookbookPdf.ts` y los componentes de lookbook para cliente y administrador.
...
1. Rediseñar únicamente el generador PDF de `src/lib/lookbookPdf.ts` para que el archivo descargado se parezca al PDF de referencia `CATÁLOGO DIGITAL.pdf`.
